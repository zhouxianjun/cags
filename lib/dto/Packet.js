    ...
    write(socket, user, start){
        if(socket){
            ...
            if(this._logger){
                ...
                socket.remoteAddress, socket.remotePort, user, this.cmd, this.getResult().code,
                    this.getResult().msg, buffer.length, this._resultByte.toBuffer().length, this.body ? this.body.toBuffer().length : 0,
